...
func (uq *UserQuery) QueryChatRooms() *ChatRoomQuery {
	query := (&ChatRoomClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, user.ChatRoomsTable, user.ChatRoomsColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) WithChatRooms(opts ...func(*ChatRoomQuery)) *UserQuery {
	query := (&ChatRoomClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withChatRooms; query != nil {
		if err := uq.loadChatRooms(ctx, query, nodes, nil,
			func(n *User, e *ChatRoom) { n.Edges.ChatRooms = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadChatRooms(ctx context.Context, query *ChatRoomQuery, nodes []*User, init func(*User), assign func(*User, *ChatRoom)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].chat_room_users == nil {
			continue
		}
		fk := *nodes[i].chat_room_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chat_room_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
