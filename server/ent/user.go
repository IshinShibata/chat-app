...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldUID, user.FieldEmail, user.FieldName:
			values[i] = new(sql.NullString)
		case user.FieldCreatedAt, user.FieldUpdatedAt, user.FieldLastLoginAt:
			values[i] = new(sql.NullTime)
		case user.ForeignKeys[0]: // chat_room_users
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field chat_room_users", value)
			} else if value.Valid {
				u.chat_room_users = new(int)
				*u.chat_room_users = int(value.Int64)
			}
		default:
